...
A mobile-first web application that enables seamless self-service check-in for in-person community event attendees through a simple three-screen flow accessible via event-specific URLs.
...
**Primary Users:** In-person community event attendees who need to check-in at physical events
...
- **Event-Specific URLs:** Unique URLs for each event following pattern `https://codingwithai.chkin.io/eventid/`
...
- **Real-time Raffle System:** Support for raffle winner announcements with polling for live updates
- **Community Branding:** Dynamic logos, banners, and host information display
- **Venue Information Display:** Show event location details and amenities post-check-in
...
- **Favicon System:** Database field exists but UI implementation deferred
- **Advanced Admin Interface:** Complex event management features beyond basic CRUD
...
- **As an attendee,** I want to see if I've won any raffles so I can claim prizes during the event
...
- **As an organizer,** I want to run raffles and announce winners in real-time to boost engagement
...
- **Community Engagement:** Community organizers will manage their own branding and content
- **Raffle Usage:** Communities will use the raffle system responsibly for legitimate prizes
...
- **Supabase:** Database hosting, API, and backend infrastructure including Edge Functions for raffle system
- **Vercel:** Frontend hosting and deployment platform
- **Domain Management:** Access to configure codingwithai.chkin.io domain
...
- **Tailwind CSS:** CSS framework with custom theming system using CSS custom properties